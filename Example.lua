...
function library:CreateSettingsTab(menu)
    ...
    local settings = menu:AddTab("Settings"); do 
        ...
        local main_section = settings:AddSection("Main", 2); do
	    	...
	    	setclipboard("discord.gg/6bck9MPMEz")
                    library:SendNotification("Copied Invite", 3);
            end});
            main_section:AddButton({text = "Rejoin Server", confirm = true, callback = function()
                game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId);
            end});
            main_section:AddButton({text = "Rejoin Game", confirm = true, callback = function()
                game:GetService("TeleportService"):Teleport(game.PlaceId);
            end})
            main_section:AddBind({text = "Open / Close", flag = "togglebind", nomouse = true, noindicator = true, bind = Enum.KeyCode.M, callback = function()
                library:SetOpen(not library.open)
            end});
        end;
    end;
